    ...
    export default function CartModal({ isOpen, onClose, items, onUpdateQuantity, onRemoveItem }: CartModalProps) {
      ...
      return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-start justify-end">
          <div className="bg-[#FFF5E6] w-full max-w-md h-full overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-[#FF6B00]">Seu pedido em</h2>
                  <h3 className="text-xl text-[#8B4513]">Mini Kalzone Unifor</h3>
                </div>
                <button onClick={onClose} className="text-[#FF6B00] hover:text-[#FF8533]">
                  <X size={24} />
                </button>
              </div>

              <div className="mb-6">
                <h4 className="text-[#8B4513] font-medium mb-4">Kalzones (não alteramos ingredientes)</h4>
                {items.map((item) => (
                  <div key={item.id} className="mb-4 pb-4 border-b border-[#FFE4C4]">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-[#8B4513]">{item.quantity}x {item.name}</span>
                      <span className="text-[#8B4513]">R$ {(item.price * item.quantity).toFixed(2)}</span>
                    </div>
